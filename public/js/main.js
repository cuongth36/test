$(document).ready(function() {
    //  SLIDER
    var slider = $('#slider-wp .section-detail');
    slider.owlCarousel({
        autoPlay: 4500,
        navigation: false,
        navigationText: false,
        paginationNumbers: false,
        pagination: true,
        items: 1, //10 items above 1000px browser width
        itemsDesktop: [1000, 1], //5 items between 1000px and 901px
        itemsDesktopSmall: [900, 1], // betweem 900px and 601px
        itemsTablet: [600, 1], //2 items between 600 and 0
        itemsMobile: true // itemsMobile disabled - inherit from itemsTablet option
    });

    //  ZOOM PRODUCT DETAIL
    $("#zoom").elevateZoom({ gallery: 'list-thumb', cursor: 'pointer', galleryActiveClass: 'active', imageCrossfade: true, loadingIcon: 'http://www.elevateweb.co.uk/spinner.gif' });

    //  LIST THUMB
    var list_thumb = $('#list-thumb');
    list_thumb.owlCarousel({
        navigation: true,
        navigationText: false,
        paginationNumbers: false,
        pagination: false,
        stopOnHover: true,
        items: 5, //10 items above 1000px browser width
        itemsDesktop: [1000, 5], //5 items between 1000px and 901px
        itemsDesktopSmall: [900, 5], // betweem 900px and 601px
        itemsTablet: [768, 5], //2 items between 600 and 0
        itemsMobile: true // itemsMobile disabled - inherit from itemsTablet option
    });

    //  FEATURE PRODUCT
    var feature_product = $('#feature-product-wp .list-item');
    feature_product.owlCarousel({
        autoPlay: true,
        navigation: true,
        navigationText: false,
        paginationNumbers: false,
        pagination: false,
        stopOnHover: true,
        items: 4, //10 items above 1000px browser width
        itemsDesktop: [1000, 4], //5 items between 1000px and 901px
        itemsDesktopSmall: [800, 3], // betweem 900px and 601px
        itemsTablet: [600, 2], //2 items between 600 and 0
        itemsMobile: [375, 1] // itemsMobile disabled - inherit from itemsTablet option
    });

    //  SAME CATEGORY
    var same_category = $('#same-category-wp .list-item');
    same_category.owlCarousel({
        autoPlay: true,
        navigation: true,
        navigationText: false,
        paginationNumbers: false,
        pagination: false,
        stopOnHover: true,
        items: 4, //10 items above 1000px browser width
        itemsDesktop: [1000, 4], //5 items between 1000px and 901px
        itemsDesktopSmall: [800, 3], // betweem 900px and 601px
        itemsTablet: [600, 2], //2 items between 600 and 0
        itemsMobile: [375, 1] // itemsMobile disabled - inherit from itemsTablet option
    });

    //  SCROLL TOP
    $(window).scroll(function() {
        if ($(this).scrollTop() != 0) {
            $('#btn-top').stop().fadeIn(150);
        } else {
            $('#btn-top').stop().fadeOut(150);
        }
    });
    $('#btn-top').click(function() {
        $('body,html').stop().animate({ scrollTop: 0 }, 800);
    });

    // CHOOSE NUMBER ORDER
    var value = parseInt($('#num-order').attr('value'));
    $('#plus').click(function() {
        let order = $('#num-order-wp #num-order');
        let numberOrder = parseInt(order.attr('max'));
        if (value >= numberOrder) {
            value = numberOrder;
            $(this).addClass('disabled');
        } else {
            value++;
        }
        $('#num-order').attr('value', value);
    });
    $('#minus').click(function() {
        let plus = $('#plus');
        if (value > 1) {
            plus.removeClass('disabled');
            value--;
            $('#num-order').attr('value', value);
        }
    });

    //AJAX UPDATE CART
    $('.minus-change').click(function(e) {
        e.preventDefault();
        var parent = $(this).closest('.product-info');
        var element = $(this).closest('.cart-row');
        var qty = parseInt($('.number-order-quantity', element).attr('value'));
        var plusChange = $('.plus-change', element);
        if (qty > 1) {
            qty--;
            plusChange.removeClass('disabled');
        }
        console.log(qty);
        var action = $('.action-update-cart', element),
            url = action.data('action'),
            row = action.data('row'),
            id = action.data('id'),
            token = $('meta[name="csrf-token"]').attr('content'),
            data = { id: id, row: row, qty: qty, _token: token };
        $.ajax({
            url: url,
            method: 'post',
            data: data,
            dataType: 'JSON',
            success: function(data) {
                $('.total', element).text(data.total);
                $('#total-price .sub-total').text(data.sub_total);
                $('.number-order-quantity', element).attr('value', qty);
            },
            error: function(xhr, ajaxOption, throwError) {
                alert(xhr.status);
                alert(throwError);
            }
        });
    });

    $('.plus-change').click(function(e) {
        e.preventDefault();
        var parent = $(this).closest('.product-info');
        var element = $(this).closest('.cart-row');
        var qty = parseInt($('.number-order-quantity', element).attr('value'));
        var action = $('.action-update-cart', element);
        var url = action.data('action');
        var row = action.data('row');
        var id = action.data('id');
        var max = parseInt($('.number-order-quantity', element).data('max'));
        var token = $('meta[name="csrf-token"]').attr('content');
        console.log(max);
        if (qty < max) {
            qty++;
        } else if (qty == max) {
            $(this).addClass('disabled');
        }
        var data = { id: id, row: row, qty: qty, _token: token };

        $.ajax({
            url: url,
            method: 'post',
            data: data,
            dataType: 'JSON',
            success: function(data) {
                $('.total', element).text(data.total);
                $('#total-price .sub-total').text(data.sub_total);
                $('.number-order-quantity', element).attr('value', qty);
            },
            error: function(xhr, ajaxOption, throwError) {
                alert(xhr.status);
                alert(throwError);
            }
        });
    });


    //Ajax add to cart form homepage
    var numberOrderCart = $('.btn-cart .num').text();
    $('.action .add-cart').click(function(e) {
        e.preventDefault();
        let current = $(this);
        let url = current.attr('href');
        let id = current.data('id');
        let token = $('meta[name="csrf-token"]').attr('content');
        let data = { id: id, _token: token };
        numberOrderCart++;
        $([document.documentElement, document.body]).animate({
            scrollTop: $('#cart-wp').offset().top
        }, 2000);
        $('.fa.fa-spin', current).css('display', 'inline-block');
        $.ajax({
            url: url,
            method: 'get',
            data: data,
            dataType: 'text',
            success: function(data) {
                $('.fa.fa-spin', current).css('display', 'none');
                $('.btn-cart .num').text(numberOrderCart);
                $('.desc #number-product').text(numberOrderCart);
                // $('.list-add-cart').html(data);
                $('.cart-info').html(data);

            },
            error: function(xhr, ajaxOption, throwError) {
                alert(xhr.status);
                alert(throwError);
            }
        });
    });


    // Ajax add cart single product
    $('#add-to-cart').click(function(e) {
        e.preventDefault();
        let current = $(this);
        let parent = $(this).closest('.form-action-cart');
        let url = current.data('action');
        let id = $('#product-id', parent).data('id');
        let qty = $('#num-order', parent).val();
        let token = $('meta[name="csrf-token"]').attr('content');
        let numberOrder = $('.btn-cart .num').text();
        let order = $('#num-order', parent).val();
        let totalOrder = parseInt(numberOrder) + parseInt(order);
        let data = { id: id, qty: qty, _token: token };
        $([document.documentElement, document.body]).animate({
            scrollTop: $('#cart-wp').offset().top
        }, 2000);
        $('.fa.fa-spin', current).css('display', 'inline-block');
        console.log(order);
        console.log(numberOrderCart);
        console.log(qty);
        $.ajax({
            url: url,
            method: 'get',
            data: data,
            dataType: 'text',
            success: function(data) {
                $('.fa.fa-spin', current).css('display', 'none');
                $('.btn-cart .num').text(totalOrder);
                $('.desc #number-product').text(totalOrder);
                $('.qty .result').text(totalOrder);
                $('.list-add-cart').html(data);
            },
            error: function(xhr, ajaxOption, throwError) {
                alert(xhr.status);
                alert(throwError);
            }
        });
    });




    // Delete cart
    // $('.product-cart .cart-delete').click(function(e) {
    //     e.preventDefault();
    //     var parent = $(this).closest('.cart-row');
    //     var parent_id = parent.data('id');
    //     var action = $('.del-product', parent);
    //     var id = action.data('id');
    //     var url = action.attr('href');
    //     var token = $('meta[name="csrf-token"]').attr('content');
    //     var data = { rowId: id, _token: token };
    //     var sub_total = $("#total-price .sub-total").text();
    //     console.log(sub_total);
    //     if (confirm('Bạn có chắc chắn muốn xóa sản phẩm không?')) {
    //         $.ajax({
    //             url: url,
    //             method: 'GET',
    //             data: data,
    //             dataType: 'json',
    //             success: function(data) {
    //                 if (data.rowId == parent_id) {
    //                     $('.cart-row[data-id="' + data.rowId + '"]').remove();
    //                 }
    //             },
    //             error: function(xhr, ajaxOption, throwError) {
    //                 alert(xhr.status);
    //                 alert(throwError);
    //             }
    //         });
    //     }
    // });

    //  MAIN MENU
    $('#category-product-wp .list-item > li').find('.sub-menu').after('<i class="fa fa-angle-right arrow" aria-hidden="true"></i>');

    //  TAB
    tab();

    //  EVEN MENU RESPON
    $('html').on('click', function(event) {
        var target = $(event.target);
        var site = $('#site');

        if (target.is('#btn-respon i')) {
            if (!site.hasClass('show-respon-menu')) {
                site.addClass('show-respon-menu');
            } else {
                site.removeClass('show-respon-menu');
            }
        } else {
            $('#container').click(function() {
                if (site.hasClass('show-respon-menu')) {
                    site.removeClass('show-respon-menu');
                    return false;
                }
            });
        }
    });

    //  MENU RESPON
    $('#main-menu-respon li .sub-menu').after('<span class="fa fa-angle-right arrow"></span>');
    $('#main-menu-respon li .arrow').click(function() {
        if ($(this).parent('li').hasClass('open')) {
            $(this).parent('li').removeClass('open');
        } else {

            //            $('.sub-menu').slideUp();
            //            $('#main-menu-respon li').removeClass('open');
            $(this).parent('li').addClass('open');
            //            $(this).parent('li').find('.sub-menu').slideDown();
        }
    });
});

function tab() {
    var tab_menu = $('#tab-menu li');
    tab_menu.stop().click(function() {
        $('#tab-menu li').removeClass('show');
        $(this).addClass('show');
        var id = $(this).find('a').attr('href');
        $('.tabItem').hide();
        $(id).show();
        return false;
    });
    $('#tab-menu li:first-child').addClass('show');
    $('.tabItem:first-child').show();
}